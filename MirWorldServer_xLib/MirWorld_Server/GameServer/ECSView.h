...
//    ECSRegistry::m_mutex 的共享锁 (SRLock) 或独占锁 (SWLock),
//    且在整个迭代期间不得释放锁。
//    原因:
//    1. 构造时通过 get_pool 获取池指针, 若无锁另一线程 ensure_pool 可能 realloc pools_ → 悬空
//    2. 迭代时通过 primary->entities() 获取裸指针, 若无锁另一线程 emplace/remove 可能
...
//    持全局共享锁可阻止 ensure_pool (需独占锁), 保证 pools_ 与池指针稳定;
//    但无法阻止其他线程操作不同池 (设计意图, 不同池可并发)。
//    若需阻止对同一池的并发修改, 调用者应持独占锁 (SWLock)。
//
//  用法:
//    // 单线程或已持锁场景:
//    SRLock lock(registry.m_mutex);  // 必须持锁
...
private:
	...
	// 迭代期间调用方持全局锁, 绕过池级 SRWLOCK 走 nolock 路径
	bool has_all(entity_t e) const noexcept
	{
		return has_all_impl_nolock<0>(e);
	}

	template<size_t I>
	bool has_all_impl_nolock(entity_t e) const noexcept
	{
		if constexpr (I >= sizeof...(Include))
			return true;
		else
		{
			using T = std::tuple_element_t<I, std::tuple<Include...>>;
			if constexpr (std::is_same_v<T, NullComponent>)
				return has_all_impl_nolock<I + 1>(e);
			else
			{
				auto* p = std::get<I>(pools_);
				if (!p || !p->has_nolock(e)) return false;
				return has_all_impl_nolock<I + 1>(e);
			}
		}
	}
	...
	// 获取组件引用 (回调期间使用)
	// 迭代期间调用方持全局锁, 走 get_nolock 绕过池级 SRWLOCK。
	...
	auto& get_ref(entity_t e)
	{
		...
		{
			return *std::get<I>(pools_)->get_nolock(e);
		}
	}
