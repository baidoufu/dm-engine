...
std::shared_ptr<BTNode> XMLParser::ParseFile(const std::wstring& filePath)
{
    ...
    return ParseString(std::string(xml.begin(), xml.end()));
}

std::shared_ptr<BTNode> XMLParser::ParseString(const std::string& xmlContent)
{
    // 转换为宽字符串
    std::wstring xml(xmlContent.begin(), xmlContent.end());
...
std::wstring XMLParser::ReadFileGBK(const std::wstring& path)
{
    ...
    // GBK 到宽字符转换（Windows 上使用 MultiByteToWideChar）
    int wlen = MultiByteToWideChar(936, 0, buffer.c_str(), (int)size, nullptr, 0);
    std::wstring result(wlen, L'\0');
    MultiByteToWideChar(936, 0, buffer.c_str(), (int)size, &result[0], wlen);
    return result;
}
...
BTResult BTEngine::ExecuteProbability(std::shared_ptr<BTNode> node)
{
    ...
    int chance = it != node->params.end() ? std::stoi(it->second) : 50;
...
BTResult BTEngine::ExecuteDecoratorRepeat(std::shared_ptr<BTNode> node)
{
    ...
    int count = it != node->params.end() ? std::stoi(it->second) : 1;
...
BTResult BTEngine::ExecuteCondition(std::shared_ptr<BTNode> node)
{
    ...
    case ConditionType::LOW_HP:
    {
        ...
        int pct = it != node->params.end() ? std::stoi(it->second) : 50;
    ...
    case ConditionType::TARGET_DISTANCE:
    {
        ...
        int min = itMin != node->params.end() ? std::stoi(itMin->second) : 0;
        int max = itMax != node->params.end() ? std::stoi(itMax->second) : 10;
    ...
    {
        auto it = node->params.find(L"count");
        int cnt = it != node->params.end() ? std::stoi(it->second) : 0;
